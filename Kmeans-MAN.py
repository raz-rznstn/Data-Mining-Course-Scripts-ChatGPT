...
def manhattan_distance(a, b):
    ...
    for i in range(DIM):
        ...
        calc_str += f"{diff:.2f}" # shows the calculated value of the ABS()
        if i != DIM - 1:
            calc_str += " + "

    calc_str += f" = {dist:.2f}"

    return dist, calc_str


def print_table(distances, calculations):
    print("+----+--------------------------------------------+--------------------------------------------+--------+")
    print("| Pt | Distance to C1                             | Distance to C2                             | Assign |")
    print("+----+--------------------------------------------+--------------------------------------------+--------+")
    ...
    for i in range(N):
        ...
        print(f"| P{i + 1:<2}| {c1_str:<43}| {c2_str:<43}|  C{assigned + 1}    |")
...
def kmeans():
    ...
    while changed:
        ...
        for i in range(N):
            for c in range(K):
                distances[i][c], calculations[i][c] = manhattan_distance(points[i], centroids[c])
...
def main():
    print(f"{GREEN}Starting K-Means Clustering (Manhattan Distance)...{RESET}")
    kmeans()
